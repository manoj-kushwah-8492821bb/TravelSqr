...
exports.searchHotel = async (req, res) => {
    try {
        const { cityCode, checkInDate, checkOutDate, adults, roomQuantity, currencyCode } = req.query;

        // Validation
        if (!cityCode || !checkInDate || !checkOutDate || !adults) {
            return res.status(400).json({
                ResponseCode: 400,
                ResponseMessage: 'Missing required parameters: cityCode, checkInDate, checkOutDate, adults',
                succeeded: false,
            });
        }
        ...
        const hotelIds = hotelListResponse.result.data?.map(hotel => hotel.hotelId) || [];
        ...
        for (const hotelId of hotelIds) {
            ...
            try {
                ...
                const errorDetails = err.response?.result?.errors || [{ detail: err.message }];
        ...
        if (!results.length) {
            return res.status(400).json({
                ResponseCode: 400,
                ResponseMessage: 'No available hotel offers found for the specified city and dates',
                ResponseBody: errors,
                succeeded: false,
            });
        }
...
exports.bookHotel = async (req, res) => {
    ...
    try {
        ...
        if (!hotelOffer?.offerId || !guests?.length || !payment_method) {
            ...
            return res.status(400).json({
                ResponseCode: 400,
                ResponseMessage: 'Missing required fields: hotelOffer.offerId, guests, payment_method',
                succeeded: false,
                ResponseBody: {}
            });
        }
        ...
        try {
            ...
            return res.status(400).json({
                ...
                succeeded: false,
                ResponseBody: amadeusErr.response?.data?.errors || amadeusErr.message || 'Unknown Amadeus error'
            });
        }
        ...
        try {
            ...
            const emailMessage = `
                ...
                <p><strong>Total:</strong> ${totalAmount.toFixed(2)} ${hotelOffer.price.currency}</p>
                <p><strong>Coins Earned:</strong> ${coinsEarned}</p>
            `;
            await SendMailBooking(guests[0].email, 'Hotel Booking Confirmation', emailMessage);
...
exports.getHotelBookings = async (req, res) => {
    ...
    try {
        ...
        const bookings = await HotelBookingModel.find({ user_id: req.userData._id })
            ...
            .select('booking_id hotel_name check_in_date check_out_date amount payment_status booking_status booking_date_time');
