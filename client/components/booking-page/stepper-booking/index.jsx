...
const Index = (props) => {
  ...
  const [coinsToUse, setCoinsToUse] = useState(userCoins);
  ...
  // const Currency = "$"; // Example currency symbol
  ...
  const onSubmit = async (values, { resetForm }) => {
    let status = 1;
    passCount.forEach((item) => {
      if (!item.complete) status = 0;
    });
  
    if (status === 0) {
      setMainError(`Hey, we'll need ${passCount.length - countUser} passenger details to continue.`);
      return;
    } else {
      setMainError("");
      setLoading(true);
      ...
      if (data.length > 0) {
        let travellerData = data.map((item, index) => ({
          id: index + 1,
        ...
        // ✅ Calculate Total Price After Discounts & Coins
        let baseTotal = paymentResult?.price?.grandTotal ? Number(paymentResult.price.grandTotal) : 0;
        let discountAmount = discount && discount.amount ? Number(discount.amount) : 0;
        let coinsDiscount = useCoins && coinsToUse ? Number(coinsToUse) / 100 : 0;
  
        let finalTotal = baseTotal - discountAmount - coinsDiscount;
        if (finalTotal < 0) finalTotal = 0; // Ensure total isn't negative

        let coinsToSend = useCoins && coinsToUse ? coinsToUse : 0;

      
        console.log("coinsToSend_:", coinsToSend)
        let sendData = {
          priceData: {
            ...paymentResult,
            totalAmount: finalTotal.toFixed(2), // ✅ Corrected total amount
          },
          travellerData: travellerData,
          payment_method: "card",
          coinsUsed: coinsToSend, // ✅ Send used coins only if applied
        };
  
        // ✅ Store Coins Before API Call
        if (useCoins) {
          localStorage.setItem("coinsUsed", coinsToUse);
        } else {
          localStorage.removeItem("coinsUsed");
        }
        ...
        if (promiseToken.error) {
          ...
          router.push({
            ...
            query: {
              ...
              clientsecret: promiseToken.response.ResponseBody.paymentIntentClientSecret,
  ...
  async function getUser() {
    ...
    const promise = await getRequestToken("user/auth", {}, "get", token);
    setUserCoins(promise.response.ResponseBody.coins);
  }
  ...
  // apply promo code or discount coupons
  const applyPromoCode = async () => {
    ...
    try {
      const promise = await getRequestToken(
        `user/validate-promo?promoCode=${encodeURIComponent(promoCode.trim())}`, 
        null, // No body in GET request
        "get",
        localStorage.getItem('userToken')
      );
  
      console.log("validate_promo_:", promise)
      if (promise.response?.succeeded) {
        toast.success(promise.response.ResponseMessage);
      // Store discount details in state
      setDiscount({
        amount: promise.response.discount,
        type: promise.response.discountType,
      });
      } else {
        toast.error(promise.response.ResponseMessage || "Invalid promo code!");
      }
    } catch (error) {
  ...
  const unapplyPromoCode = () => {
    setPromoCode(""); // Clear promo code input
    setDiscount({ amount: 0, type: "" }); // Reset discount state
    toast.info("Promo code removed successfully!");
  };
  
  const calculateFinalTotal = () => {
    let finalTotal = total.gt; // Original grand total
  
    // Apply promo code discount
    if (discount) {
      if (discount.type === "flat") {
        finalTotal -= discount.amount; // Flat discount
      } else if (discount.type === "percentage") {
        finalTotal -= (finalTotal * discount.amount) / 100; // Percentage discount
      }
    }
  
    // Apply coins usage only if the checkbox is checked
    if (useCoins && userCoins >= 100) {
      finalTotal -= 1; // Subtract $1 (100 coins)
    }
  
    return Math.max(0, finalTotal).toFixed(2); // Ensure no negative values & format to 2 decimals
  };
  
  // Update coinsToUse based on the checkbox and available userCoins
const handleCoinsChange = () => {
  if (useCoins) {
    setCoinsToUse(100); // Set to 100 coins when checkbox is checked (or another value based on your logic)
  } else {
    setCoinsToUse(0); // Reset to 0 when unchecked
  }
};

// Trigger the function when the checkbox is toggled
useEffect(() => {
  handleCoinsChange();
}, [useCoins]);
  ...
  useEffect(()=>{
    if(typeof window !== "undefined" && localStorage.getItem('data') !== null){
      const storedData = JSON.parse(localStorage.getItem('data'));
      setLocalStorageData(storedData)
      if(localStoragedata.length===data.length){
        setData(storedData)
      }
    }
  },[]);
                    ...
                    <div className="row">
                      <div className="col-xl-8">
                        <div className="_299Nm">
                                            ...
                                            <div className="row x-gap-20 y-gap-20 pt-20">
                                              ...
                                              <div className="col-md-4">
                                                ...
                                                <input
                                                  ...
                                                  // }
                                                  defaultValue={
                                                    localStoragedata[index]?.firstname
                                                  }
                                              ...
                                              <div className="col-md-4">
                                                ...
                                                <input
                                                  ...
                                                  // }
                                                  defaultValue={
                                                    localStoragedata[index]?.lastname
                                                  }
                                                  type="text"
                                                  className="paymentinput"
                                                />
                                              ...
                                              <div className="col-md-4">
                                                ...
                                                {/* <div className="paymentinput"> */}
                                                <input
                                                  defaultValue={localStoragedata[index]?.dateOfBirth}
                                                  type="date"
                                                  autoComplete="off"
                                                  ...
                                                  // }
                                                  selected={
                                                    localStoragedata[index]?.dateOfBirth
                                                  }
                                                  onChange={(e) => {
                                                    changeDOB(
                                                      e.target.value,
                                              ...
                                              <div className="col-md-4">
                                                ...
                                                <Select
                                                  ...
                                                  options={options}
                                                  defaultValue={localStoragedata[index]?.nationality}
                                                  onChange={(value) => {
                                                    changeNationality(
                                                      value,
                                                      index
                                              ...
                                              <div className="col-md-4">
                                                ...
                                                <Select
                                                  className="selectFull"
                                                  options={options}
                                                  defaultValue={localStoragedata[index]?.issueCountry}
                                                  // value={valueIssue}
                                                  onChange={(value) => {
                                                    changeIssueCountry(
                                                      value,
                                              ...
                                              <div className="col-md-4">
                                                ...
                                                <input
                                                  ...
                                                  // }
                                                  defaultValue={localStoragedata[index]?.issuanceDate}
                                                  selected={
                                                    localStoragedata[index]?.issuanceDate
                                                  }
                                                  onChange={(e) => {
                                                    changeIssuanceDate(
                                                      e.target.value,
                                              ...
                                              <div className="col-md-4">
                                                ...
                                                <input
                                                  ...
                                                  defaultValue={localStoragedata[index]?.expiryDate}
                                                  selected={
                                                    localStoragedata[index]?.expiryDate
                                                  }
                                                  // value={
                          ...
                          <div className="kgzwR">
                            <p>
                              By booking with Travel Square, you agree to{" "}
                              <span>
                                <a
                            ...
                            <div className="_2M1pt">
                              ...
                              <div className="_3hzoy">
                                <div className="_2ulf4">
                                  ...
                                  <div className="_3cNYf">
                                    <div className="toUhe">
                                      ...
                                      <input
                                          ...
                                          disabled={userCoins < 100} // Disable checkbox if user has less than 100 coins
                                        />
                                    </div>
                                  </div>
                                      ...
                                      <div className="_218j6">
                                        <div className="_3EDTI">
                                          <div className="_3JtXg">
                                            ...
                                            <span 
                                              className="_1i2d8" 
                                              onClick={discount && discount.amount > 0 ? unapplyPromoCode : applyPromoCode}
                                            >
                                              {discount && discount.amount > 0 ? "Remove" : "Apply"}
                                            </span>
                                          </div>
                                        </div>
                                      </div>
                                  ...
                                  {/* Show Coins Usage If Applied */}
                                  {useCoins && userCoins >= 100 && (
                                    <div className="_3cNYf">
                                      <div className="toUhe"> Coins Used
