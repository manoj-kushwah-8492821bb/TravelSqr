...
const BookingTable = () => {
  ...
  const getData = async () => {
    const promiseToken = await getRequestToken(
      `user/booking/list?page=${page}&limit=${pageLength}&search=${search}&status=${apiStatus}`,
      {},
      "get",
  ...
  useEffect(() => {
    getData();
  }, [search, page, pageLength, apiStatus, status]);
  ...
  const onCancle = (data) => {
    let a = moment(data?.booking_data?.flightOffers[0]?.itineraries[0]?.segments[0]?.departure?.at);
    let b = moment();
    let millisecondsDifference = a.diff(b);
    let hoursDifference = Math.floor(millisecondsDifference / (1000 * 60 * 60));
    //console.log(hoursDifference);
    if(Number(hoursDifference)<6){
      swal({
        title: `You can't cancle this booking!`,
        icon: "info",
        dangerMode: false,
      ...
      swal({
        title: `Are you sure you want to cancle following booking?`,
        text: `From: ${getAirPN(data?.booking_data?.flightOffers[0]
          ?.itineraries[0]?.segments[0]?.departure
          ?.iataCode)} to\n To: ${getAirPN(data?.booking_data?.flightOffers[0]
            ?.itineraries[0]?.segments[
            data?.booking_data?.flightOffers[0]
              ?.itineraries[0]?.segments?.length - 1
          ]?.arrival?.iataCode)}\n Date: ${moment(
            data?.booking_data?.flightOffers[0]
              ?.itineraries[0]?.segments[0]?.departure?.at
          ).format("MMM Do YYYY, h:mm a")}\n Refundable Amount: ${data?.booking_data?.flightOffers[0]?.price
            ?.currency} ${data?.refundable_amount}`,
        icon: "warning",
        buttons: {
          cancel: {
            text: "No",
      ...
      <div className="tabs -underline-2 js-tabs">
        ...
        <div className="tabs__content pt-30 js-tabs-content">
          <div className="tabs__pane -tab-item-1 is-tab-el-active">
            <div className="overflow-scroll scroll-bar-2" style={{ maxHeight: '400px', overflowY: 'auto' }}>
                  ...
                  {loader === true ? (
                    ...
                    bookingData.map((item, index) => {
                      return (
                        <>
                          <tr key={index + 1}>
                            {/* <td>{`${item?.passengers_info[0]?.name?.firstName} ${item?.passengers_info[0]?.name?.lastName}`}</td> */}
                            <td>
                              {getAirPN(
                            ...
                            {activeTab == 0 && (
                              <td>
                                {item?.booking_status == "confirmed" ? (
                                  ...
                                  <span
                                    className={`rounded-100 py-4 px-10 text-center text-14 fw-500 bg-green-3 text-green-2`}
                                  >
                                    Refund_in_progress
                                  </span>
                                ) : null}
                            ...
                            <td style={{
                              ...
                              >
                                <FaXmark
                                  className="iconcolor"
                                  onClick={() => onCancle(item)}
                                  size="25"
                                ></FaXmark>
                              </Tooltip>:
                                <FaXmark
                                  className="iconcolordisable"
                                  size="25"
                                ></FaXmark>
                              }
                            </td>
