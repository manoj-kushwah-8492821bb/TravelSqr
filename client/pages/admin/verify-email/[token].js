...
import ChangePasswordAdmin from "../../../components/common/ChangePasswordAdmin";
...
const SignUp = () => {
  ...
  const vefifyToken = async (tokenData) => {
    ...
    if (promise.error) {
      if(promise?.error?.response?.data?.ResponseCode==401){
        setMessage("Link expired. Please click following link to generate new link.");
        setStatus(true);
      }else{
        ...
        <div className="container">
          <div className="row justify-center">
            <div className="col-xl-6 col-lg-7 col-md-9">
              ...
              :
              <>{loading === true ?<ChangePasswordAdmin token={token}/>:<div className="col-12 d-flex align-items-center justify-content-center"><Spinner color="primary"></Spinner></div>}</>}
              </div>
            </div>
          </div>
        </div>
